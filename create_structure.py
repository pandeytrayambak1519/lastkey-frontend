...
roots = ['src/assets/fonts','src/assets/icons','src/assets/illustrations','src/assets/images']
...
# Create directories
for folder in [
    'public/images','src/api','src/assets/fonts','src/assets/icons','src/assets/illustrations','src/assets/images',
    'src/components/ui','src/components/branding','src/components/layout','src/components/landing','src/components/auth','src/components/dashboard','src/components/documents','src/components/nominees','src/components/emergency','src/components/notifications','src/components/security','src/components/profile','src/components/shared',
    'src/context','src/hooks','src/pages/public','src/pages/auth','src/pages/dashboard','src/pages/documents','src/pages/nominees','src/pages/emergency','src/pages/notifications','src/pages/security','src/pages/profile','src/pages/admin','src/pages/errors','src/routes','src/schemas','src/services','src/store','src/styles','src/utils','src/config',
]:
    (root / folder).mkdir(parents=True, exist_ok=True)

for rel, text in root_texts.items():
    path = root / rel
    if not path.exists():
...
def create_js(path):
    ...
    if path.suffix == '.js':
        if path.name == 'routeConfig.js':
            content = 'export const routeConfig = [];\n'
        else:
            content = 'export const ' + path.stem + ' = {};\n'
    else:
        comp = path.stem
...
for rel in js_components + js_hooks + styles + utils + pages:
    path = root / rel
    path.parent.mkdir(parents=True, exist_ok=True)
    create_js(path)

for folder, names in component_sets.items():
    for name in names:
        path = root / f'{folder}/{name}.jsx'
        create_js(path)

print('Structure created or updated successfully.')
